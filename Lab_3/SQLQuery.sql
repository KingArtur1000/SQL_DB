...
SELECT AVG(YEAR(GETDATE()) - Cars.YearOfManufacture) AS Avg_Exploitation_Years
FROM Cars;


-- 2. Отыскать количество автомобилей каждой марки
SELECT Brands.BrandName, COUNT(Cars.CarID) AS CarCount
FROM Cars
...
SELECT Drivers.DriverID, Drivers.LastName, Drivers.FirstName, SUM(Trips.Distance) AS TotalDistance
FROM Trips
...
SELECT COUNT(DISTINCT Trips.DriverID) AS DriversInCurrentMonth
FROM Trips
...
SELECT TOP 1 Trips.DriverID, Trips.Distance
FROM Trips
ORDER BY Trips.Distance DESC;


-- 6. Определить число поездок каждого водителя в прошлом месяце
SELECT Drivers.DriverID, Drivers.LastName, Drivers.FirstName, COUNT(Trips.TripID) AS TripCount
FROM Trips
JOIN Drivers ON Trips.DriverID = Drivers.DriverID
...
SELECT TripDate, COUNT(DISTINCT CarID) AS TripCount
FROM Trips
GROUP BY TripDate;
